...
public class VhclSrchController {
    ...
    @PostMapping("/srchVhcl")
    public Map<String, Object> srchVhcl(@RequestBody Map<String, String> request){
        ...
        // 01. 등록정보 조회
        CegCarMig vhclInfo = vhclSrchService.getVhclInfo(carRegNo);
        System.out.println("@@vhclInfo: " + vhclInfo);

        if(vhclInfo != null){
            // 02. 검사정보 목록 조회 (INDIVIDUAL_INSPECTION)
            inspInfoList = vhclSrchService.getInspInfoList(carRegNo);

            // 03. chatGPT API 호출 - !!추후 수정
            apiInfo = vhclSrchService.callGptApi(vhclInfo);
        }
